...
// P_SetPsprite
//
void P_SetPspriteRef(player_t& player, pspdef_t& psp, int32_t stnum)
{
	do
	{
		if (!stnum)
...
void P_SetPsprite(player_t& player, int position, int32_t stnum)
{
	...
	P_SetPspriteRef(player, player.psprites[position], stnum);
}

//
// A_FireSound
//
void A_FireSound (const player_t& player, const char *sound)
{
...
//
bool P_EnoughAmmo(const player_t& player, weapontype_t weapon, bool switching = false)
{
...
void A_Punch(AActor* mo)
{
	...
	int damage = (P_Random (player.mo)%10+1)<<1;
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_FIST);

	const int slope = P_AimLineAttack(player.mo, angle, player.mo->info->meleerange);
...
void A_Saw(AActor* mo)
{
	...
	const int damage = 2 * (P_Random (player.mo)%10+1);
	angle_t angle = player.mo->angle;
	angle += P_RandomDiff(player.mo) << 18;
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_CHAINSAW);
...
void A_FireMissile(AActor* mo)
{
	...
	if (serverside)
	{
		M_LogWDLProjectileFire(player, player.mo->angle / 4, MOD_ROCKET);
...
void A_FireBFG(AActor* mo)
{
	...
	if (serverside)
	{
		...
		M_LogWDLProjectileFire(player, player.mo->angle / 4, MOD_BFG_BOOM);
	}
...
void A_FirePlasma(AActor* mo)
{
	...
	if (serverside)
	{
		M_LogWDLProjectileFire(player, player.mo->angle / 4, MOD_PLASMARIFLE);
...
void A_FireRailgun(AActor* mo)
{
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_RAILGUN);
...
void A_FirePistol(AActor* mo)
{
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_PISTOL);
}
...
void A_FireShotgun(AActor* mo)
{
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_SHOTGUN);
}
...
void A_FireShotgun2(AActor* mo)
{
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_SSHOTGUN);
}
...
void A_FireCGun(AActor* mo)
{
	...
	M_LogWDLAccuracyShot(player, player.mo->angle / 4, MOD_CHAINGUN);
}
...
void A_BFGSpray(AActor* mo)
{
	...
	if (mo->target->player)
	{
		M_LogWDLAccuracyShot(*mo->target->player, mo->target->player->mo->angle / 4,
		                     MOD_BFG_SPLASH);
	}
