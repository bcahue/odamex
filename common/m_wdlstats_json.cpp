...
//
// DESCRIPTION:
//   WDLStats v6 (GameV6) -> JSON serialization. Lives in common (next to the
//   recorder / aggregator) so both the on-disk parity dump (M_CommitWDLLog) and
//   the server's upload path can share it. The JSON shape is the v6 contract; the
//   System.Text.Json-compatible formatting (TimeSpan "c", DateTimeOffset +00:00)
//   is reproduced here so the output diffs clean against the C# parser.
//
//-----------------------------------------------------------------------------
...
// Build the current/just-finished match's v6 JSON (compiled live in g_liveGame).
// Returns an empty string when nothing was recorded. A fresh matchId (RFC 4122
// v4) is minted per call for upload idempotency (§6.3).
